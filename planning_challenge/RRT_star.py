"""
RRT* Algorithm Implementation for 3-DOF Robotic Arm
"""
...
def run_rrt_star_example():
    """Run RRT* algorithm with visualization"""
    ...
    if success:
        ...
        print("Note: RRT* typically produces more optimal paths than basic RRT")
        
        # Create visualization
        visualize_rrt_star_results(planner, start_state, goal_state, obstacles, path)
    else:
        print("No path found. Try increasing max_iter or adjusting obstacles.")

def visualize_rrt_star_results(planner: Planner, start: JointState, goal: JointState, 
                              obstacles: list[ObstType], path: list[JointState]):
    """Visualize RRT* results with plots and animation"""
    ...
    ax1.grid(True)
    ax1.set_title('RRT* - Optimized Path')
    ...
    ax1.plot(path_x, path_y, 'g-', alpha=0.7, linewidth=2, label='Optimized Path')
    ...
    def animate(frame):
        ...
        ax2.set_aspect('equal')
        ax2.grid(True)
        ax2.set_title(f'RRT* - Animation Frame {frame}/{len(path)}')
        ax2.set_xlabel('X position')
        ax2.set_ylabel('Y position')
        ...
        if frame > 0:
            ...
            ax2.plot(current_path_x, current_path_y, 'g-', alpha=0.5, linewidth=2, label='Optimized Path')
    ...
    # Create animation
    anim = FuncAnimation(fig, animate, frames=len(path), interval=200, repeat=True, blit=False)
    
    plt.tight_layout()
    plt.show()

if __name__ == "__main__":
    run_rrt_star_example()