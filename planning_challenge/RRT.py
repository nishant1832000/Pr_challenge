...
def run_rrt_example():
    ...
    if success:
        ...
        visualize_rrt_results(planner, start_state, goal_state, obstacles, path)
    else:
        print("No path found. Try increasing max_iter or adjusting obstacles.")

def visualize_rrt_results(planner: Planner, start: JointState, goal: JointState, 
    ...
    ax1.plot(path_x, path_y, 'g-', alpha=0.7, linewidth=2, label='Planned Path')
    ...
    def animate(frame):
        ...
        if frame > 0:
            ...
            ax2.plot(current_path_x, current_path_y, 'g-', alpha=0.5, linewidth=2, label='Path')
